...
setting file, with several approaches including from scratch.
For reference, there are
existing settings files in PYPIT-development suite.
The relevant settings files are located in::

    PYPIT-development-suite/pypit_files/

No matter how you generate your Settings Files,
we **strongly recommend** that you generate a separate
file for each instrument setup.

Settings File from pypit_setup
++++++++++++++++++++++++++++++

If you have run pypit_setup (*recommended*), then
the code will have generated a .pypit file.  You
can simply copy this file to a new .pypit file,
renaming it according to instrument setup (*recommended*)
or however you prefer to organize your reductions, e.g.::

    cp lris_blue_2016-Nov-19.pypit lris_blue_setup_01_02.pypit
    or
    cp lris_blue_2016-Nov-19.pypit lris_blue_600_4000_long1.0.pypit

If you wish to restrict the reduction to a specific instrument
setup (*recommended*), add the following line to the Settings File::

    reduce setup SETUP_ID

where SETUP_ID is the setup identifier.  For an instrument
with a single detector (e.g. Lick/Kast), this will be a two digit
number, e.g. 01.  For instruments with multiple detectors, you
will likely wish to combine those, e.g. 01_02 for Keck/LRIS.
Refer to the .group file in your working directory (if you have
generated one, e.g. with pypit_setup).

Calibration Check
+++++++++++++++++

The Settings File generated by pypit_setup will have the following
line in it::

    run calcheck True

This is runs the code to organize the calibration and science
files, but no further.

At this stage, you should reassign filetype or remove files
from the reduction process.  You reassign file type by
adding an entry in the spect read/end block::
...
+++++++++++++++++++++++++++
Now let's look at how to make one from scratch.
