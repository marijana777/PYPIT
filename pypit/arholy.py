...
def run_holy2(tcent, idpix, idwave, npix, llist, noncalib=None, ngrid=100,
    ...
    ----------
    tcent : ndarray
      Centroid of lines found in spectrum
    idpix : ndarray
      Centroids of identified lines (Holy 1)
    idwave : ndarray
...
def holy2_brute(tcent, idpix, idwave, npix, llist):
    ...
    coeff = arutils.func_fit(idyval, idwave, "polynomial", 1)
    wmnmx = arutils.func_val(coeff, np.array([0.0,1.0]), "polynomial")
    wmin, wmax = wmnmx[0], wmnmx[1]

    wavmean = np.mean(idwave) #waves[wmsk])
    xval = (idwave - wavmean)/(wmax-wmin)
    ll = (llist - wavmean)/(wmax-wmin)
    ...
    # pixels.
    coeff = arutils.func_fit(xval, idyval, "polynomial", 3)
